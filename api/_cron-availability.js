...
export default async function handler(req, res) {
  ...
  // accepted for manual triggering. Previously this endpoint had no
  ...
  try {
    ...
    // Load all providers (names — manual availability toggle removed 2026-08-16)
    const { data: providers } = await supabase
      .from('providers')
      .select('id, first_name, last_name, is_provider')
      .eq('is_active', true)
    const provMap = {}
