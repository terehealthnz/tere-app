...
export async function createOfferTemplate({ name, roleTitleDefault, compensationDefault, contractTerms, sortOrder, contractPdfBase64, contractPdfName, contractVersion, feePerConsult } = {}) {
  const res = await apiFetch('/api/job-applications?action=create_offer_template', {
    method: 'POST',
    body: JSON.stringify({ name, roleTitleDefault, compensationDefault, contractTerms, sortOrder, contractPdfBase64, contractPdfName, contractVersion, feePerConsult }),
  })
