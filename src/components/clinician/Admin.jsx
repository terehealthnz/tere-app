...
function OverviewDashboard({ setAdminTab }) {
  ...
  return (
    <>
      {/* NHI-first patient lookup — NZ admin only. Wrapped in isNZ() so
          the whole widget block (including the "Find patient:" label) is
          gone on terecare.com AND on tere.co.nz (AU beta — Medicare is
          the AU identifier, not NHI). Every query writes a nhi_query row
          to audit_logs. See docs/quality-management-system.md §7.4. */}
      {isNZ() && (
          ...
          <div onClick={e => e.stopPropagation()} style={{ background:'white', borderRadius:14, width:'100%', maxWidth:640, boxShadow:'0 20px 60px rgba(0,0,0,.3)' }}>
            ...
            <div style={{ padding:'1.25rem 1.5rem', fontSize:'.875rem', color:'#374151' }}>
              <div style={{ marginBottom:'.875rem' }}>
                ...
                <div>{open.patient_first_name} {open.patient_last_name}{isNZ() && ` · NHI ${open.patient_nhi || '—'}`} · DOB {open.patient_dob || '—'}</div>
              </div>
              <div style={{ marginBottom:'.875rem' }}>
                    ...
                    <div style={{ fontWeight:700, fontSize:'.9375rem', color:'#0D2B45', marginBottom:2, display:'flex', alignItems:'center', flexWrap:'wrap', gap:6 }}>
                      ...
                      {p.id === currentProviderId && !p.practice_only && (
                        <button
                          ...
                          title={sandboxOn ? 'Turn sandbox off in your own view (does not affect the server lock)' : 'Turn sandbox on for your own view (does not lock other sessions)'}
                          style={{
                          ...
                          {sandboxOn ? '🧪 Exit my sandbox' : '🧪 My sandbox'}
                        </button>
                      )}
                    </div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:'1rem', alignItems:'start' }}>
                  <div>
                    ...
                    <div style={{ fontSize:'.8125rem', color:'#9CA3AF' }}>
                      ...
                      {isNZ() && r.patient_nhi && ` · NHI: ${r.patient_nhi}`}
                    </div>
                    {r.result_notes && (
